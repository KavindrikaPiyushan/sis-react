...
const UniversityLogin = ({ setRole }) => {
  ...
  return (
  <div
    className="min-h-screen flex items-center justify-center p-2 sm:p-4 lg:p-6 relative overflow-hidden"
    style={{
  ...
  >
    ...
    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-48 sm:w-72 lg:w-96 h-48 sm:h-72 lg:h-96 rounded-full blur-2xl sm:blur-3xl animate-pulse" style={{ background: branding.orbGradients[2], opacity: branding.orbOpacities[2], animationDelay: branding.orbDelays[2] }}></div>
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className={`absolute top-16 sm:top-20 left-8 sm:left-20 ${mounted ? 'animate-bounce' : ''} transform hover:scale-110 transition-transform`} 
        ...
        <div className={`absolute top-20 sm:top-32 right-12 sm:right-32 ${mounted ? 'animate-bounce' : ''} transform hover:scale-110 transition-transform`} 
        ...
        <div className={`hidden sm:block absolute bottom-32 left-32 ${mounted ? 'animate-bounce' : ''} transform hover:scale-110 transition-transform`} 
             style={{ color: '#004477', opacity: 0.4, animationDelay: '2s', animationDuration: '3s' }}>
          <Users className="w-7 h-7 filter drop-shadow-lg" />
        </div>
        <div className={`hidden lg:block absolute top-1/2 right-20 ${mounted ? 'animate-bounce' : ''} transform hover:scale-110 transition-transform`} 
             style={{ color: '#003355', opacity: 0.4, animationDelay: '1.5s', animationDuration: '4s' }}>
          <Sparkles className="w-5 h-5 filter drop-shadow-lg" />
        </div>
      </div>

      {/* Main Login Card with enhanced animations - responsive design */}
      <div className="bg-white/90 backdrop-blur-2xl rounded-2xl sm:rounded-3xl shadow-2xl shadow-black/15 p-4 sm:p-6 lg:p-8 w-full max-w-sm sm:max-w-md relative z-10 border border-white/30 transform-gpu animate-in zoom-in-95 slide-in-from-bottom-4 duration-700 ease-out mx-2">
        ...
        <div className="absolute -inset-0.5 rounded-2xl sm:rounded-3xl blur opacity-60 animate-pulse" style={{ background: 'linear-gradient(90deg, #004488 0%, #003366 50%, #002244 100%)' }}></div>
        
        <div className="relative bg-[rgb(228,243,255)]/95 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-8 -m-4 sm:-m-6 lg:-m-8">
          {/* Header with staggered animations - responsive typography */}
          <div className="text-center mb-6 sm:mb-8">
            <div className="relative mb-3 sm:mb-4 animate-in zoom-in-50 duration-1000 delay-300">
              <div className="w-12 sm:w-16 h-12 sm:h-16 rounded-xl sm:rounded-2xl flex items-center justify-center mx-auto shadow-2xl transform transition-all duration-300" style={{ backgroundColor: '#003366', boxShadow: '0 25px 50px -12px rgba(0, 51, 102, 0.3)' }}>
                <School className="w-6 sm:w-8 h-6 sm:h-8 text-white filter drop-shadow-lg" />
              </div>
            </div>
            
            <h1 className="text-xl sm:text-2xl font-bold bg-clip-text text-transparent mb-2 animate-in slide-in-from-top-2 duration-700 delay-500" style={{ background: branding.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              Welcome Back
            </h1>
            <div className="space-y-1 text-gray-600 animate-in fade-in duration-700 delay-700">
              <p className="font-semibold text-gray-800 transform hover:scale-105 transition-transform text-sm sm:text-base">{branding.system}</p>
              <p className="text-xs sm:text-sm">{branding.faculty} - {branding.university}</p>
            </div>
          </div>
          ...
          {/* Login Form with staggered animations - responsive spacing */}
          <div className="space-y-4 sm:space-y-6">
            <div className="animate-in slide-in-from-left-4 duration-500 delay-800">
              ...
              <div className={`relative group transform transition-all duration-300 ${focusedField === 'email' ? 'scale-105' : ''}`}>
                <div className="absolute inset-y-0 left-0 pl-3 sm:pl-4 flex items-center pointer-events-none">
                  <Mail className={`w-4 sm:w-5 h-4 sm:h-5 transition-all duration-300 ${focusedField === 'email' ? 'scale-110' : ''}`} 
                ...
                <input
                  ...
                  className="w-full pl-10 sm:pl-12 pr-3 sm:pr-4 py-2.5 sm:py-3 text-sm sm:text-base border border-gray-200 rounded-lg sm:rounded-xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all duration-300 bg-white/60 hover:bg-white/80 focus:bg-white hover:shadow-lg focus:shadow-xl transform focus:scale-[1.02]"
            ...
            <div className="animate-in slide-in-from-right-4 duration-500 delay-900">
              ...
              <div className={`relative group transform transition-all duration-300 ${focusedField === 'password' ? 'scale-105' : ''}`}>
                <div className="absolute inset-y-0 left-0 pl-3 sm:pl-4 flex items-center pointer-events-none">
                  <Lock className={`w-4 sm:w-5 h-4 sm:h-5 transition-all duration-300 ${focusedField === 'password' ? 'scale-110' : ''}`}
                ...
                <input
                  ...
                  onBlur={() => setFocusedField('')}
                  className="w-full pl-10 sm:pl-12 pr-10 sm:pr-12 py-2.5 sm:py-3 text-sm sm:text-base border border-gray-200 rounded-lg sm:rounded-xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all duration-300 bg-white/60 hover:bg-white/80 focus:bg-white hover:shadow-lg focus:shadow-xl transform focus:scale-[1.02]"
                ...
                <button
                  ...
                  className="absolute inset-y-0 right-0 pr-3 sm:pr-4 flex items-center transition-all duration-300 transform hover:scale-110"
                  style={{ color: '#9CA3AF' }}
            ...
            <button
              onClick={handleLogin}
              disabled={isLoading}
              className="w-full text-white font-semibold py-3 sm:py-4 px-4 sm:px-6 rounded-lg sm:rounded-xl flex items-center justify-center gap-2 sm:gap-3 transition-all duration-300 shadow-2xl hover:shadow-2xl hover:-translate-y-1 disabled:transform-none disabled:shadow-lg animate-in slide-in-from-bottom-4 duration-500 delay-1100 transform-gpu group text-sm sm:text-base"
              style={{
            ...
            >
              {isLoading ? (
                <div className="relative">
                  ...
                  <div className="absolute inset-0 w-4 sm:w-5 h-4 sm:h-5 border-2 border-transparent rounded-full animate-spin" style={{ borderTopColor: '#004488', animationDirection: 'reverse', animationDuration: '0.8s' }}></div>
                </div>
              ) : (
                <LogIn className="w-4 sm:w-5 h-4 sm:h-5 transform group-hover:scale-110 group-hover:rotate-3 transition-transform" />
              )}
          ...
          <div className="text-center pt-3 sm:pt-4 mt-3 sm:mt-4 border-t border-gray-200/60 text-gray-500 text-xs space-y-1 animate-in fade-in duration-500 delay-1300">
      ...
      {/* Enhanced Forgot Password Modal - responsive design */}
      {showForgotModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-lg flex items-center justify-center z-50 p-2 sm:p-4 animate-in fade-in duration-300">
          <div className="bg-white/95 backdrop-blur-2xl rounded-xl sm:rounded-2xl shadow-2xl w-full max-w-sm sm:max-w-md transform transition-all animate-in zoom-in-95 slide-in-from-bottom-4 duration-500 ease-out border border-white/20 mx-2">
            <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200/60">
              ...
              <button
                ...
                className="p-1.5 sm:p-2 hover:bg-gray-100/80 rounded-lg sm:rounded-xl transition-all duration-300 text-gray-400 hover:text-gray-600 transform hover:scale-110 hover:rotate-90"
              >
            ...
            <div className="p-4 sm:p-6">
              <p className="text-gray-600 mb-4 sm:mb-6 leading-relaxed animate-in fade-in duration-500 delay-200 text-sm sm:text-base">
              ...
              <div className="animate-in slide-in-from-left-4 duration-500 delay-300">
                ...
                <input
                  ...
                  className="w-full px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-200 rounded-lg sm:rounded-xl focus:ring-4 outline-none transition-all duration-300 bg-white/70 hover:bg-white/90 focus:bg-white transform hover:scale-[1.02] focus:scale-[1.02] text-sm sm:text-base"
                  onFocus={(e) => {
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 justify-end p-4 sm:p-6 border-t border-gray-200/60 animate-in fade-in duration-500 delay-400">
              <button
                onClick={() => setShowForgotModal(false)}
                className="px-4 sm:px-6 py-2 sm:py-2.5 text-gray-700 border border-gray-200 rounded-lg sm:rounded-xl hover:bg-gray-50/80 transition-all duration-300 font-medium transform hover:scale-105 hover:-translate-y-0.5 text-sm sm:text-base order-2 sm:order-1"
              >
                Cancel
              </button>
              <button
                onClick={handleForgotPassword}
                className="px-4 sm:px-6 py-2 sm:py-2.5 text-white rounded-lg sm:rounded-xl transition-all duration-300 font-medium shadow-lg transform hover:scale-105 hover:-translate-y-0.5 text-sm sm:text-base order-1 sm:order-2"
                style={{ 
